...
from pathlib import Path  # A) robust paths
...
def get_explainer(_trained_classifier, feature_names):
    # C) avoid SHAP warning; let it choose a safe mode automatically
...
THRESHOLD_IMG_PATH = RESULTS_DIR / "threshold_curve.png"  # A) robust path
if THRESHOLD_IMG_PATH.exists():
    st.sidebar.image(str(THRESHOLD_IMG_PATH), caption="Threshold sweep (F1 score)", use_container_width=True)
else:
...
with st.sidebar.expander("BMI Calculator", expanded=False):
    ...
    if bmi_calc is not None and np.isfinite(bmi_calc):
        ...
        if st.button("Use this BMI"):
            ...
            # (B) remove extra st.rerun(); button click already triggers rerun
...
if st.session_state["show_prediction"]:
    # D) spinner around the whole compute path
    with st.spinner("Working…"):
        ...
        try:
            ...
            try:
                ...
                # A) fix Arrow dtype errors by casting to string
                display_df["Feature"] = display_df["Feature"].astype(str)
                display_df["Your value"] = display_df["Your value"].astype(str)
                display_df["Effect on risk"] = display_df["Effect on risk"].astype(str)
